...
router.get('/', async (req, res) => {
  try {
    ...
    (transacoesAnteriores || []).forEach(t => {
      ...
      if (t.conta === 'CONTA_0') {
        t.tipo === 'ENTRADA' ? saldoAnterior0 += valor : saldoAnterior0 -= valor;
      } else if (t.conta === 'CONTA_6') {
        t.tipo === 'ENTRADA' ? saldoAnterior6 += valor : saldoAnterior6 -= valor;
      }
    });
...
router.post('/', async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.from('fluxo_caixa').insert([payload]).select().single();
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.from('fluxo_caixa').update(payload).eq('id', id).select().single();
    if (error) throw error;
    res.json(data);
  } catch (error) {
    res.status(500).json({ error: error.message });
...
router.post('/conciliar', async (req, res) => {
  try {
    const { ids } = req.body;
    if (!ids || ids.length === 0) return res.status(400).json({ error: 'Nenhuma selecionada.' });

    const { data: transacoes } = await supabase.from('fluxo_caixa').select('*').in('id', ids);
    ...
    await supabase.from('fluxo_caixa').update({ conciliado: true, codigo_conciliacao: codigo }).in('id', ids);
    ...
    if (spread !== 0) {
      ...
      const dataMaisRecente = new Date(Math.max(...datas)).toISOString().split('T')[0];
...
router.post('/desconciliar', async (req, res) => {
  try {
    ...
    await supabase.from('fluxo_caixa').delete().eq('codigo_conciliacao', codigo).eq('categoria', 'LUCRO / SPREAD');
    await supabase.from('fluxo_caixa').update({ conciliado: false, codigo_conciliacao: null }).eq('codigo_conciliacao', codigo);
...
router.post('/fechar-mes', async (req, res) => {
  try {
    ...
    const { data } = await supabase.from('fechamentos_caixa').insert([{ mes, saldo_conta_0, saldo_conta_6 }]).select().single();
    res.status(201).json(data);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
