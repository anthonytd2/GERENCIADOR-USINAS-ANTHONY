...
export default function BalancoEnergeticoCard({ mesFiltro }: Props) {
  ...
  if (loading || !dados) return <div className="h-96 bg-gray-50-card rounded-lg animate-pulse flex items-center justify-center text-gray-500">Calculando Balanço Histórico...</div>;
    ...
    <div className="bg-gray-50-card p-6 rounded-lg shadow-sm border border-gray-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="space-y-2">
          <p className="text-xs font-bold text-gray-500 uppercase">Geração (Usinas)</p>
          ...
          <p className="text-xs text-gray-500 px-2">Real: <b>{dados.realizado.geracao.toLocaleString('pt-BR')} kWh</b></p>
        </div>

        <div className="space-y-2">
          <p className="text-xs font-bold text-gray-500 uppercase">Consumo (Carteira)</p>
          ...
          <p className="text-xs text-gray-500 px-2">Real: <b>{dados.realizado.consumo.toLocaleString('pt-BR')} kWh</b></p>
        </div>

        <div className="space-y-2">
          <p className="text-xs font-bold text-gray-500 uppercase">Saldo Operacional</p>
          ...
          <p className="text-xs text-gray-500 px-2">Saldo Real: <b className={dados.saldo.realizado < 0 ? 'text-red-500' : ''}>{dados.saldo.realizado.toLocaleString('pt-BR')} kWh</b></p>
        </div>
      </div>

      {/* GRÁFICO VERTICAL MÊS A MÊS */}
      <div className="border-t border-gray-200 pt-6">
        <h4 className="text-xs font-bold text-gray-500 uppercase mb-6 flex items-center gap-2">
