  ...
  return (
    <div className="max-w-7xl mx-auto pb-20">
      <div className="mb-8 border-b border-gray-200 pb-6 flex justify-between items-center">
        ...
        <Link to="/propostas" className="flex items-center gap-2 text-gray-500 hover:text-gray-900 bg-gray-100 px-4 py-2 rounded-lg">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-gray-50-card p-5 rounded-xl shadow-sm border border-gray-200">
            <h2 className="font-bold text-gray-900 mb-4 flex items-center gap-2">
              <User className="w-5 h-5 text-blue-600" /> Cliente
            </h2>
            <div className="flex bg-gray-100 p-1 rounded-lg mb-4">
              <button onClick={() => setTipoCliente('base')} className={`flex-1 py-1.5 text-sm  rounded-md transition-all ${tipoCliente === 'base' ? 'bg-gray-50-card shadow text-blue-700' : 'text-gray-500'}`}>Base</button>
              <button onClick={() => setTipoCliente('prospect')} className={`flex-1 py-1.5 text-sm  rounded-md transition-all ${tipoCliente === 'prospect' ? 'bg-gray-50-card shadow text-blue-700' : 'text-gray-500'}`}>Novo</button>
            </div>
            {tipoCliente === 'base' && (
              <div className="mb-4">
                <select className="w-full p-2 border rounded-lg bg-gray-50-card text-sm" value={clienteSelecionadoId} onChange={handleClienteChange}>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <select name="concessionaria_id" value={form.concessionaria_id} onChange={handleInputChange} className="w-full p-2 border rounded text-sm bg-gray-50-card">
          ...
          <form onSubmit={handleSimular} className="bg-gray-50-card p-5 rounded-xl shadow-sm border border-gray-200">
            <h2 className="font-bold text-gray-900 mb-4 flex items-center gap-2">
            ...
            <div className="space-y-3">
              <div>
                <label className="text-xs font-bold text-gray-500">Consumo Total (kWh)</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] font-bold text-gray-500 mb-0.5">TUSD (Valor R$)</label>
                  <CurrencyInput
                    name="valorTusd"
                    prefix="R$ "
                    decimalSeparator=","
                ...
                <div>
                  <label className="block text-[10px] font-bold text-gray-500 mb-0.5">TE (Valor R$)</label>
                  <CurrencyInput
                    name="valorTe"
                    prefix="R$ "
                    decimalSeparator=","
                ...
                <div>
                  <label className="block text-[10px] font-bold text-gray-500 mb-0.5">Bandeira (R$)</label>
                  <CurrencyInput
                    name="valorBandeira"
                    prefix="R$ "
                    decimalSeparator=","
                ...
                <div>
                  <label className="block text-[10px] font-bold text-gray-500 mb-0.5">Ilum. Púb (R$)</label>
                  <CurrencyInput
                ...
                <div>
                  <label className="block text-[10px] font-bold text-gray-500 mb-0.5">Outros (R$)</label>
                  <CurrencyInput
                    name="valorOutros"
                    prefix="R$ "
                    decimalSeparator=","
        ...
        <div className="lg:col-span-8">
          {!resultado ? (
            <div className="h-full flex flex-col items-center justify-center bg-gray-50 border-2 border-dashed border-gray-200 rounded-xl text-gray-500 min-h-[400px]">
            ...
            <div className="space-y-6 animate-fade-in">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-green-600 text-white p-6 rounded-xl shadow-sm relative overflow-hidden">
                  ...
                  <p className="text-green-100 text-sm  mb-1">Economia Real (Seu Bolso)</p>
                  <h3 className="text-3xl font-bold">{fmt(resultado.economiaRealCliente)}</h3>
                  <p className="text-xs mt-2 opacity-80">Por mês</p>
                </div>
                <div className="bg-gray-50-card p-6 rounded-xl shadow border border-gray-200">
                  <p className="text-gray-500 text-sm  mb-1">% Redução Fatura Total</p>
                  <h3 className="text-3xl font-bold text-blue-600">{fmtP(resultado.percentualReducaoTotal)}</h3>
                  <p className="text-xs text-gray-500 mt-2">Sobre o custo total atual</p>
                </div>
                <div className="bg-gray-50-card p-6 rounded-xl shadow border border-gray-200">
                  <p className="text-gray-500 text-sm  mb-1">Pagamento à Usina</p>
                  <h3 className="text-3xl font-bold text-gray-900">{fmt(resultado.pagamentoUsina)}</h3>
                  <p className="text-xs text-gray-500 mt-2">Assinatura Mensal</p>
                </div>
              </div>

              {/* Tabela Comparativa (Tela) */}
              <div className="bg-gray-50-card rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <div className="p-4 bg-gray-50 border-b flex justify-between items-center">
                  <h3 className="font-bold text-gray-900">Raio-X da Economia</h3>
                  <span className="text-xs bg-gray-50-card px-2 py-1 border rounded text-gray-500">Consumo: {form.consumoKwh} kWh</span>
                </div>
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    <tr><td className="px-6 py-3 text-gray-500">TUSD (Fio B + Encargos)</td><td className="px-6 py-3 text-right">{fmt(resultado.dadosOriginais.valorTusd)}</td><td className="px-6 py-3 text-right text-orange-600 ">{fmt(resultado.detalhes.novoTusd)} *</td></tr>
                    <tr><td className="px-6 py-3 text-gray-500">TE (Energia Elétrica)</td><td className="px-6 py-3 text-right">{fmt(resultado.dadosOriginais.valorTe)}</td><td className="px-6 py-3 text-right text-green-600 font-bold">0,00</td></tr>
                    <tr><td className="px-6 py-3 text-gray-500">Bandeira Tarifária</td><td className="px-6 py-3 text-right">{fmt(resultado.dadosOriginais.valorBandeira)}</td><td className="px-6 py-3 text-right text-green-600 font-bold">0,00</td></tr>
              ...
              <div className="flex gap-4 pt-4">
                <button onClick={() => salvarProposta('Rascunho')} className="flex-1 py-3 border border-gray-200 text-gray-700  rounded-lg hover:bg-gray-50 flex justify-center items-center gap-2">
                  <Save className="w-4 h-4" /> Salvar como Nova Versão
                </button>
                <button onClick={gerarPDF} className="flex-[2] py-3 bg-green-600 hover:bg-green-700 text-white font-bold rounded-lg shadow-sm flex justify-center items-center gap-2">
