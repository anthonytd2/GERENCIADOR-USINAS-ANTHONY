...
export default function DetalheUsina() {
  ...
  const logradouro = usina.endereco || usina.endereco_proprietario;

  if (logradouro) partesEndereco.push(logradouro);
  if (usina.bairro) partesEndereco.push(`Bairro ${usina.bairro}`);
  if (usina.cidade) partesEndereco.push(usina.cidade);
  if (usina.uf) partesEndereco.push(usina.uf);

  const enderecoReal = partesEndereco.length > 0
    ? partesEndereco.join(', ') + (usina.cep ? ` - CEP: ${usina.cep}` : '')
  ...
  return (
    <div className="space-y-8 animate-fade-in-down pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {/* CARD 3: PROPRIETÁRIO E LOCALIZAÇÃO */}
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold uppercase border ${v.status?.descricao === 'Ativo'
