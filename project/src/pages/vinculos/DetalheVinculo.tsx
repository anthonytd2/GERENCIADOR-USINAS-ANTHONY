...
// IMPORTANTE: Importamos o cliente JÁ CONFIGURADO com suas chaves
...
export default function DetalheVinculo() {
  ...
  const uploadArquivo = async (file: File, bucketName: string) => {
    ...
    // USAMOS O CLIENTE IMPORTADO AQUI
    ...
    <div className="max-w-6xl mx-auto pb-20 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 relative overflow-hidden">
          ...
          <p className="text-lg font-bold text-gray-900 line-clamp-1" title={nomeConsumidor}>{nomeConsumidor}</p>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 relative overflow-hidden">
          ...
          <p className="text-lg font-bold text-gray-900 line-clamp-1" title={nomeUsina}>{nomeUsina}</p>
        ...
        <div className="bg-white rounded-xl border border-blue-100 shadow-md p-6 animate-fade-in-down">
          ...
          <form onSubmit={handleSalvar} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-6">
            <div className="lg:col-span-1">
              ...
              <input required type="date" className="w-full border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500" value={formData.mesreferencia} onChange={e => setFormData({...formData, mesreferencia: e.target.value})} />
            </div>
            <div className="lg:col-span-1">
              <label className="block text-xs font-bold text-gray-600 uppercase mb-1">Energia (kWh)</label>
              <input required type="number" className="w-full border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500" value={formData.energiacompensada} onChange={e => setFormData({...formData, energiacompensada: e.target.value})} />
            </div>
            <div className="lg:col-span-1">
            ...
            <div className="lg:col-span-2">
              ...
              <input readOnly type="number" className="w-full bg-gray-100 border-gray-200 rounded-lg text-blue-700 font-bold cursor-not-allowed" value={formData.spread} />
            </div>
            <div className="lg:col-span-3 p-4 bg-gray-50 rounded-lg border border-dashed border-gray-300 relative">
              <label className="block text-sm font-medium text-gray-700 mb-2 flex items-center gap-2"><FileText size={16} className="text-blue-500"/> Planilha / Fatura</label>
              {formData.arquivourl_existente ? (
                ...
                <input type="file" className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-blue-100 file:text-blue-700 hover:file:bg-blue-200" onChange={e => setFormData({...formData, arquivo: e.target.files ? e.target.files[0] : null})} />
              )}
            </div>
            <div className="lg:col-span-3 p-4 bg-gray-50 rounded-lg border border-dashed border-gray-300 relative">
              <label className="block text-sm font-medium text-gray-700 mb-2 flex items-center gap-2"><Upload size={16} className="text-green-500"/> Comprovante / Recibo</label>
              {formData.recibourl_existente ? (
                ...
                <input type="file" className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-green-100 file:text-green-700 hover:file:bg-green-200" onChange={e => setFormData({...formData, recibo: e.target.files ? e.target.files[0] : null})} />
              )}
            </div>
            ...
            <tbody className="divide-y divide-gray-50">
              {fechamentos.map((f) => (
                <tr key={f.fechamentoid} className="hover:bg-blue-50/50 transition-colors group">
                  <td className="px-6 py-4 font-medium text-gray-900 capitalize">{f.mesreferencia ? new Date(f.mesreferencia).toLocaleDateString('pt-BR', { month: 'short', year: 'numeric', timeZone: 'UTC' }) : '-'}</td>
