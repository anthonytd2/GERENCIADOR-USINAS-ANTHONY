...
export default function DetalheVinculo() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-20 animate-fade-in-down">
      ...
      <div>
        <Link to="/vinculos" className="inline-flex items-center gap-2 text-gray-500 hover:text-gray-900 mb-6 transition-colors ">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Link
          ...
          className="group relative overflow-hidden bg-gradient-to-br from-blue-600 to-blue-700 text-white p-6 rounded-lg shadow-sm shadow-blue-200 transition-all hover:shadow-xl hover:-translate-y-1"
        >
          <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
            <DollarSign size={100} />
          </div>
          <div className="relative z-10 flex items-center gap-4">
            <div className="p-3 bg-gray-50-card/20 rounded-xl backdrop-blur-sm">
        ...
        <Link
          ...
          className="group relative overflow-hidden bg-gradient-to-br from-indigo-600 to-violet-700 text-white p-6 rounded-lg shadow-sm shadow-indigo-200 transition-all hover:shadow-xl hover:-translate-y-1"
        >
          <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
            <Activity size={100} />
          </div>
      ...
      <div className="bg-gray-50-card p-6 rounded-lg shadow-sm border border-gray-200">
        ...
        {!vinculo.unidades_vinculadas || vinculo.unidades_vinculadas.length === 0 ? (
          <p className="text-gray-500 italic text-sm text-center py-4 bg-gray-50 rounded-xl border border-dashed border-gray-200">
            Nenhuma UC específica (Vínculo Global).
          </p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {vinculo.unidades_vinculadas.map((item) => (
              <div key={item.id} className="flex items-start gap-3 p-3 bg-gray-50 rounded-xl border border-gray-200 hover:border-purple-200 transition-colors relative">
      ...
      <div className="bg-gray-50-card p-6 rounded-lg shadow-sm border border-gray-200">
        <div className="flex items-center gap-2 mb-6 border-b border-gray-200 pb-2">
          ...
          <h3 className="font-bold text-gray-900">Administrativo & Contratos</h3>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="space-y-4 text-sm lg:col-span-1 border-r border-gray-200 pr-4">
            <div>
              <p className="text-gray-500 text-xs uppercase font-bold">Início</p>
              <p className=" text-gray-900">{vinculo.data_inicio ? new Date(vinculo.data_inicio).toLocaleDateString('pt-BR') : '-'}</p>
            </div>
            <div>
              <p className="text-gray-500 text-xs uppercase font-bold">Fim</p>
              <p className=" text-gray-900">{vinculo.data_fim ? new Date(vinculo.data_fim).toLocaleDateString('pt-BR') : 'Indeterminado'}</p>
            </div>
            <div>
              <p className="text-gray-500 text-xs uppercase font-bold">Local</p>
              <p className=" text-gray-900">{vinculo.consumidores?.cidade}/{vinculo.consumidores?.uf}</p>
            </div>
          </div>

          <div className="lg:col-span-2">
            <p className="text-xs font-bold text-gray-500 uppercase mb-4">Emissão de Documentos</p>
            <div className="flex flex-wrap gap-3 mb-6">
              <button
                onClick={() => gerarContratoComodatoConsumidor(vinculo)}
                className="flex-1 min-w-[200px] px-4 py-3 bg-gray-50-card border border-gray-200 text-gray-500 rounded-xl hover:bg-gray-50 hover:border-purple-300 hover:text-purple-700 transition-all  text-sm flex items-center justify-center gap-2"
              >
              ...
              <button
                ...
                className="flex-1 min-w-[200px] px-4 py-3 bg-gray-50-card border border-gray-200 text-gray-500 rounded-xl hover:bg-gray-50 hover:border-emerald-300 hover:text-emerald-700 transition-all  text-sm flex items-center justify-center gap-2"
              >
            ...
            {!isEncerrado && (
              <div className="mt-6 pt-6 border-t border-gray-200">
                <button
                  ...
                  className="text-red-500 hover:text-red-700 text-sm  flex items-center gap-2 hover:bg-red-50 px-3 py-2 rounded-lg transition-colors"
                >
