            ...
            {fechamentos.map((f) => (
              <tr key={f.fechamento_id} className="hover:bg-blue-50/20 transition-colors">
                ...
                <td className="px-6 py-4 text-right">
                  <button onClick={() => handleEditar(f)} className="text-blue-500 hover:bg-blue-50 p-2 rounded-lg transition-colors"><Edit2 size={16}/></button>
                  {/* Botão de Excluir que chama o MODAL */}
                  <button onClick={() => solicitarExclusao(f.fechamento_id)} className="text-red-500 hover:bg-red-50 p-2 rounded-lg ml-1 transition-colors"><Trash2 size={16}/></button>
                </td>
              </tr>
            ))}
          </tbody>
